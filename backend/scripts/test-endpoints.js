...
const endpointTests = [
  {
    ...
    validateResponse: data => {
      ...
      if (data.status !== 'running' && data.status !== 'OK') {
        throw new Error(
          `Expected status 'running' or 'OK', got '${data.status}'`
        );
      }
      return true;
    },
...
const rateLimitTests = [
  {
    ...
    async run() {
      ...
      if (rateLimited) {
        ...
        console.log(
          '⚠️  Rate limiting not detected (may be configured for higher limits)'
        );
      }
...
const corsTests = [
  {
    ...
    async run() {
      ...
      console.log(
        `✅ CORS Allow-Origin: ${corsHeaders['access-control-allow-origin']}`
      );

      if (corsHeaders['access-control-allow-methods']) {
        console.log(
          `✅ CORS Allow-Methods: ${corsHeaders['access-control-allow-methods']}`
        );
      }
...
async function runEndpointTests() {
  ...
  if (totalFailed > 0) {
    console.log(
      '💡 Some endpoint tests failed. Check the error messages above.'
    );
    process.exit(1);
  }
