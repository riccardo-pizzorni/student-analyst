...
async function testEndpoint(name, url, expectedStatus = 200) {
  ...
  try {
    ...
    if (result.success && result.statusCode === expectedStatus) {
      console.log(`${colors.green}✅ ${name}: OK (${result.responseTime}ms)${colors.reset}`);
      return { 
        name, 
        status: 'healthy', 
      ...
      console.log(`${colors.yellow}⚠️  ${name}: Degraded - HTTP ${result.statusCode} (${result.responseTime}ms)${colors.reset}`);
      return { 
        name, 
        status: 'degraded', 
        responseTime: result.responseTime, 
    ...
    console.log(`${colors.red}❌ ${name}: Down - ${error.error} (${error.responseTime}ms)${colors.reset}`);
    return { 
      name, 
      status: 'down', 
...
async function runMonitoringTests() {
  console.log(`${colors.blue}🔍 STUDENT ANALYST - Monitoring Health Check${colors.reset}`);
  ...
  results.forEach(result => {
    ...
    console.log(`  ${statusIcon} ${result.name}: ${statusColor}${result.status}${colors.reset} (${result.responseTime}ms)`);
  });
...
if (require.main === module) {
  runMonitoringTests().catch(error => {
    console.error(`${colors.red}❌ Monitoring test failed:${colors.reset}`, error);
    process.exit(3);
  });
}
