...
export const strictRateLimiter = rateLimit({
  ...
  handler: (req: Request, res: Response) => {
    console.warn(`🚨 Strict rate limit exceeded for IP: ${req.ip} - ${req.method} ${req.originalUrl}`);
    res.status(429).json({
      error: 'API Rate Limit Exceeded',
      message: 'You have made too many API requests. Please wait before trying again.',
      details: {
...
export const securityLogger = (req: Request, res: Response, next: NextFunction) => {
  ...
  res.end = function(...args: any[]) {
    ...
    if (res.statusCode >= 400) {
      console.warn(`⚠️  Error response: ${res.statusCode} for ${req.originalUrl} from ${req.ip}`);
    }
