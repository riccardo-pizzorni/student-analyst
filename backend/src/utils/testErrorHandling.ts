  ...
  private async testErrorClassification(): Promise<void> {
    ...
    const testCases = [
      ...
      },
      {
        ...
        error: new Error(
          'Invalid API key. Please try again or visit https://www.alphavantage.co/support/#api-key'
        ),
      ...
      },
      {
        ...
        error: new Error(
          'Invalid API call. Please retry or visit the documentation. Symbol=INVALID123 not found.'
        ),
        expectedType: SystemErrorType.SYMBOL_NOT_FOUND,
      },
      {
        name: 'Connection Timeout Error',
        error: Object.assign(new Error('Operation timeout after 30000ms'), {
          name: 'TimeoutError',
        }),
      ...
      },
      {
        ...
        error: Object.assign(new Error('ENOTFOUND api.alphavantage.co'), {
          name: 'NetworkError',
        }),
    ...
    for (const testCase of testCases) {
      ...
      try {
        ...
        console.log(
          `  ❌ ${testCase.name}: Test failed - ${(error as Error).message}`
        );
      }
    }
  ...
  private async testRecoveryStrategies(): Promise<void> {
    ...
    for (const testCase of testCases) {
      ...
      try {
        ...
        const strategyMatch = classified.recovery.strategy.includes(
          testCase.expectedStrategy.split('_')[0]
        );
  ...
  private printSummary(): void {
    ...
    if (failedTests > 0) {
      ...
      this._results
        ...
        .forEach(r => {
          console.log(
            `  - ${r.testName}: ${(r.details as { error?: string })?.error || 'Unknown error'}`
          );
        });
      console.log('');
    }
