...
export class DataSanitizationTester {
  ...
  static async runAllTests(): Promise<TestSuiteResult> {
    ...
    console.log(`   Failed: ${allTests.length - passed} ${allTests.length - passed > 0 ? '❌' : ''}`);
  ...
  private static getDateValidationTests(): TestCase[] {
    return [
      ...
      },
      {
        ...
        input: { startDate: "2023-01-01'; DROP TABLE--", endDate: '2023-12-31' },
        expected: {
          isValid: false,
          hasErrors: true
        },
  ...
  private static async runSingleTest(test: TestCase): Promise<{
    ...
    try {
      ...
      switch (test.category) {
        ...
        default:
          result = { isValid: false, errors: ['Unknown test category'], warnings: [] };
      }
  ...
  static async quickTest(): Promise<boolean> {
    ...
    try {
      // Test XSS
      const xssTest = DataSanitizer.removeDangerousPatterns('<script>alert("xss")</script>');
      if (xssTest.isValid) {
      ...
      if (htmlTest !== '&lt;div&gt;test &amp; &quot;quotes&quot;&lt;&#x2F;div&gt;') {
