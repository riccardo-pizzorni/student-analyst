...
export class AlphaVantageServiceTester {
  ...
  private async testValidation(): Promise<void> {
    await this.runTest('Input Validation - Valid Symbols', async () => {
      ...
      for (const symbol of TEST_CONFIG.validSymbols) {
        const validation = this.service.validateRequest(symbol, AlphaVantageTimeframe.DAILY);
        if (!validation.valid) {
          throw new Error(`Valid symbol ${symbol} failed validation: ${validation.errors.join(', ')}`);
        }
    ...
    await this.runTest('Input Validation - Invalid Symbols', async () => {
      ...
      for (const symbol of TEST_CONFIG.invalidSymbols) {
        ...
        results.push({ symbol, valid: validation.valid, errors: validation.errors });
      }
  ...
  private async testGetStockDataDaily(): Promise<void> {
    await this.runTest('Get Stock Data - Daily', async () => {
      ...
      this.validateStockDataResponse(response, symbol, AlphaVantageTimeframe.DAILY);
  ...
  private async testGetStockDataIntraday(): Promise<void> {
    await this.runTest('Get Stock Data - Intraday 5min', async () => {
      ...
      this.validateStockDataResponse(response, symbol, AlphaVantageTimeframe.INTRADAY_5MIN);
  ...
  private async testGetStockDataWeekly(): Promise<void> {
    await this.runTest('Get Stock Data - Weekly', async () => {
      ...
      this.validateStockDataResponse(response, symbol, AlphaVantageTimeframe.WEEKLY);
  ...
  private async testGetStockDataMonthly(): Promise<void> {
    await this.runTest('Get Stock Data - Monthly', async () => {
      ...
      this.validateStockDataResponse(response, symbol, AlphaVantageTimeframe.MONTHLY);
  ...
  private async testInvalidSymbols(): Promise<void> {
    await this.runTest('Error Handling - Invalid Symbols', async () => {
      ...
      for (const invalidSymbol of TEST_CONFIG.invalidSymbols) {
        try {
          await this.service.getStockData(invalidSymbol, AlphaVantageTimeframe.DAILY);
          throw new Error(`Expected error for invalid symbol: ${invalidSymbol}`);
        } catch (error) {
  ...
  private validateStockDataResponse(
    ...
    if (typeof firstDataPoint.volume !== 'number' || firstDataPoint.volume < 0) {
  ...
  private printTestSummary(totalTime: number): void {
    ...
    console.log('\n📈 PERFORMANCE METRICS:');
    const avgDuration = this.testResults.reduce((sum, test) => sum + test.duration, 0) / total;
    ...
    console.log(`  Cache Keys: ${cacheStats.keys.length > 0 ? cacheStats.keys.join(', ') : 'None'}`);
    
    console.log('\n🎯 RECOMMENDATIONS:');
    if (failed === 0) {
      console.log('  ✅ All tests passed! Alpha Vantage Service is working correctly.');
    } else {
    ...
    if (avgDuration > 5000) {
      console.log('  ⚡ Consider optimizing for better performance (avg > 5s per test)');
    }
