...
class ApiKeyManager {
  ...
  private getCurrentApiKey(): string {
    ...
    if (timeSinceRotation > config.rotationInterval || config.usageCount >= config.maxUsage) {
      this.rotateApiKey();
    }
  ...
  async callAlphaVantageAPI(queryParams: Record<string, string>): Promise<any> {
    ...
    try {
      ...
      } else if (queryParams.function?.includes('WEEKLY') || queryParams.function?.includes('MONTHLY')) {
  ...
  private detectSuspiciousActivity(log: ApiUsageLog): void {
    ...
    if (sameIpErrors > 5) {
      console.warn(`🚨 Suspicious activity detected from IP ${log.ip}: ${sameIpErrors} errors in 5 minutes`);
      log.suspicious = true;
    ...
    if (sameIpRequests > 50) {
      console.warn(`🚨 High request volume from IP ${log.ip}: ${sameIpRequests} requests in 5 minutes`);
      log.suspicious = true;
    }
  }
...
export class ApiProxyService {
  ...
  static async getHistoricalData(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      const result = await apiKeyManager.fetchWithCache(url, cacheKey, cacheTTL);
