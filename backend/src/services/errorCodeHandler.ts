...
import { AlphaVantageError, AlphaVantageErrorType } from './alphaVantageService';
...
const COMMON_SYMBOLS = [
  ...
  'SPY', 'QQQ', 'IWM', 'DIA', 'VTI', 'VOO', 'BND', 'TLT', 'GLD', 'SLV',
...
export class ErrorCodeHandler {
  ...
  private identifyErrorType(error: Error): SystemErrorType {
    ...
    if (message.includes('rate limit') || message.includes('25 requests per day')) {
      return SystemErrorType.DAILY_LIMIT_EXCEEDED;
    }
    if (message.includes('5 calls per minute') || message.includes('frequency')) {
      return SystemErrorType.RATE_LIMIT_EXCEEDED;
    }

    // Authentication patterns
    if (message.includes('api key') || message.includes('unauthorized') || message.includes('403') || message.includes('401')) {
    ...
    if (message.includes('network') || message.includes('enotfound') || message.includes('dns')) {
   ...
   */
  private mapAlphaVantageError(alphaVantageType: AlphaVantageErrorType): SystemErrorType {
    const mapping: Record<AlphaVantageErrorType, SystemErrorType> = {
      [AlphaVantageErrorType.API_LIMIT_EXCEEDED]: SystemErrorType.DAILY_LIMIT_EXCEEDED,
      [AlphaVantageErrorType.RATE_LIMIT]: SystemErrorType.RATE_LIMIT_EXCEEDED,
      [AlphaVantageErrorType.AUTHENTICATION_ERROR]: SystemErrorType.INVALID_API_KEY,
      [AlphaVantageErrorType.INVALID_SYMBOL]: SystemErrorType.SYMBOL_NOT_FOUND,
      [AlphaVantageErrorType.NETWORK_ERROR]: SystemErrorType.NETWORK_UNAVAILABLE,
      [AlphaVantageErrorType.NO_DATA_AVAILABLE]: SystemErrorType.NO_DATA_AVAILABLE,
      [AlphaVantageErrorType.PARSING_ERROR]: SystemErrorType.INVALID_DATA_FORMAT,
      [AlphaVantageErrorType.MALFORMED_RESPONSE]: SystemErrorType.INVALID_DATA_FORMAT,
      [AlphaVantageErrorType.INVALID_FUNCTION]: SystemErrorType.INVALID_REQUEST,
      [AlphaVantageErrorType.INVALID_TIMEFRAME]: SystemErrorType.UNSUPPORTED_TIMEFRAME
    };
   ...
   * Genera messaggio user-friendly
   */
  private generateUserFriendlyMessage(error: ClassifiedError): UserFriendlyMessage {
    const messages: Record<SystemErrorType, UserFriendlyMessage> = {
      [SystemErrorType.RATE_LIMIT_EXCEEDED]: {
        title: 'Limite di richieste temporaneo',
        message: 'Stiamo rispettando i limiti del servizio gratuito (5 richieste al minuto). La richiesta verrà elaborata automaticamente tra qualche secondo.',
        suggestion: 'Non è necessaria alcuna azione. Il sistema gestirà automaticamente l\'attesa.',
        estimatedResolution: '30-60 secondi'
      },
      [SystemErrorType.DAILY_LIMIT_EXCEEDED]: {
        title: 'Limite giornaliero raggiunto',
        message: 'Hai raggiunto il limite di 25 richieste gratuite per oggi. Il servizio si resetterà automaticamente domani alle 00:00 UTC.',
        suggestion: 'Puoi utilizzare i dati già presenti nella cache o attendere il reset di domani. Considera l\'utilizzo di più servizi API se hai bisogno di più dati.',
      ...
      [SystemErrorType.INVALID_API_KEY]: {
        ...
        message: 'Si è verificato un problema temporaneo con l\'accesso ai servizi di dati finanziari. Il sistema sta tentando di utilizzare credenziali alternative.',
        suggestion: 'Riprova tra qualche minuto. Se il problema persiste, contatta il supporto.',
      ...
      [SystemErrorType.CONNECTION_TIMEOUT]: {
        ...
        message: 'La connessione ai servizi finanziari è più lenta del normale. Il sistema sta riprovando automaticamente.',
        suggestion: 'Verifica la tua connessione internet. Se il problema persiste, i dati potrebbero essere serviti dalla cache.',
        estimatedResolution: '1-2 minuti'
      },
      [SystemErrorType.NETWORK_UNAVAILABLE]: {
        title: 'Servizio temporaneamente non disponibile',
        message: 'Il servizio di dati finanziari non è raggiungibile al momento. Il sistema sta tentando di utilizzare fonti alternative.',
        suggestion: 'Riprova tra qualche minuto. Il sistema utilizzerà dati dalla cache quando possibile.',
      ...
      [SystemErrorType.NO_DATA_AVAILABLE]: {
        ...
        suggestion: 'Verifica che il simbolo sia corretto e che il mercato sia aperto. Prova con un timeframe diverso.',
        technicalDetails: 'No market data available for requested symbol and timeframe'
      },
      [SystemErrorType.INVALID_DATA_FORMAT]: {
        title: 'Errore nel formato dei dati',
        message: 'I dati ricevuti dal servizio finanziario non sono nel formato atteso. Il sistema sta riprovando.',
        suggestion: 'Questo è un problema temporaneo del servizio esterno. Riprova tra qualche minuto.',
        estimatedResolution: '2-5 minuti'
      },
      [SystemErrorType.MARKET_CLOSED]: {
        title: 'Mercato chiuso',
        message: 'Il mercato è attualmente chiuso. I dati potrebbero non essere aggiornati in tempo reale.',
        suggestion: 'I dati mostrati sono dell\'ultima sessione di trading. Riprova durante gli orari di mercato.',
        estimatedResolution: 'Prossima apertura del mercato'
      },
      [SystemErrorType.DNS_FAILURE]: {
        title: 'Problema di connessione',
        message: 'Impossibile raggiungere il servizio di dati finanziari. Problema di rete rilevato.',
        suggestion: 'Verifica la tua connessione internet e riprova tra qualche minuto.',
        estimatedResolution: '1-5 minuti'
      },
      [SystemErrorType.SERVICE_UNAVAILABLE]: {
        title: 'Servizio non disponibile',
        message: 'Il servizio di dati finanziari è temporaneamente non disponibile per manutenzione.',
        suggestion: 'Il servizio dovrebbe tornare disponibile a breve. Riprova tra qualche minuto.',
        estimatedResolution: '10-30 minuti'
      },
      [SystemErrorType.SERVICE_OVERLOADED]: {
        title: 'Servizio sovraccarico',
        message: 'Il servizio di dati finanziari è temporaneamente sovraccarico. Il sistema sta gestendo la coda.',
        suggestion: 'Attendi qualche secondo, il sistema riproverà automaticamente.',
        estimatedResolution: '30-120 secondi'
      },
      [SystemErrorType.DATA_VALIDATION_FAILED]: {
        title: 'Dati non validi',
        message: 'I dati ricevuti non hanno superato i controlli di validità del sistema.',
        suggestion: 'Riprova con parametri diversi o contatta il supporto se il problema persiste.',
      ...
      [SystemErrorType.INVALID_REQUEST]: {
        ...
        suggestion: 'Verifica che tutti i campi siano compilati correttamente e riprova.',
        estimatedResolution: 'Immediata con parametri corretti'
      },
      [SystemErrorType.MALFORMED_SYMBOL]: {
        title: 'Simbolo malformato',
        message: 'Il simbolo inserito non rispetta il formato richiesto.',
        suggestion: 'Usa simboli nel formato corretto (es: AAPL, MSFT, BTC-USD).',
      ...
      [SystemErrorType.UNSUPPORTED_TIMEFRAME]: {
        ...
        suggestion: 'Prova con un timeframe diverso (1min, 5min, 15min, 30min, 60min, daily).',
      ...
      [SystemErrorType.INTERNAL_ERROR]: {
        ...
        suggestion: 'Il problema è stato segnalato automaticamente. Riprova tra qualche minuto.',
        estimatedResolution: '5-15 minuti'
      },
      [SystemErrorType.CACHE_ERROR]: {
        title: 'Errore cache',
        message: 'Problema temporaneo con il sistema di cache dei dati.',
        suggestion: 'I dati verranno recuperati direttamente dalla fonte. Potrebbe richiedere più tempo.',
  ...
  private generateSymbolSuggestions(invalidSymbol: string): string {
    if (!invalidSymbol) return 'Verifica che il simbolo sia scritto correttamente (es: AAPL, MSFT, GOOGL).';

    const upperSymbol = invalidSymbol.toUpperCase();
    const suggestions = COMMON_SYMBOLS.filter(symbol => {
      return symbol.includes(upperSymbol) || 
  ...
  private determineRecoveryStrategy(error: ClassifiedError): RecoveryInfo {
    const strategies: Record<SystemErrorType, RecoveryInfo> = {
      ...
      [SystemErrorType.DAILY_LIMIT_EXCEEDED]: {
        ...
        requiredUserAction: 'Attendere il reset di domani o utilizzare servizi alternativi'
      },
      [SystemErrorType.INVALID_API_KEY]: {
      ...
      [SystemErrorType.SYMBOL_NOT_FOUND]: {
        ...
        requiredUserAction: 'Verificare il simbolo e riprovare con un simbolo valido'
      },
      [SystemErrorType.MARKET_CLOSED]: {
        strategy: RecoveryStrategy.FALLBACK_TO_CACHE,
        fallbackOptions: ['cache', 'last_session_data'],
        requiredUserAction: 'I dati mostrati sono dell\'ultima sessione di trading'
      },
      [SystemErrorType.DNS_FAILURE]: {
        strategy: RecoveryStrategy.RETRY_WITH_BACKOFF,
        retryConfig: {
      ...
      [SystemErrorType.NO_DATA_AVAILABLE]: {
        strategy: RecoveryStrategy.USER_ACTION_REQUIRED,
        requiredUserAction: 'Verificare il simbolo e il timeframe, provare con parametri diversi'
      },
      [SystemErrorType.DATA_VALIDATION_FAILED]: {
        strategy: RecoveryStrategy.USER_ACTION_REQUIRED,
        requiredUserAction: 'Verificare i parametri della richiesta e riprovare'
      },
      [SystemErrorType.INVALID_REQUEST]: {
        strategy: RecoveryStrategy.USER_ACTION_REQUIRED,
      ...
      [SystemErrorType.MALFORMED_SYMBOL]: {
        strategy: RecoveryStrategy.USER_ACTION_REQUIRED,
        requiredUserAction: 'Inserire un simbolo nel formato corretto (es: AAPL, MSFT)'
      },
      [SystemErrorType.UNSUPPORTED_TIMEFRAME]: {
        strategy: RecoveryStrategy.USER_ACTION_REQUIRED,
      ...
      [SystemErrorType.UNKNOWN_ERROR]: {
        ...
        requiredUserAction: 'Contattare il supporto tecnico se il problema persiste'
      }
    };
  ...
  public async executeWithRetry<T>(
    ...
    for (let attempt = 1; attempt <= retryConfig.maxAttempts; attempt++) {
      try {
        ...
        console.log(`Retry attempt ${attempt} in ${actualDelay}ms for ${context.operation}`);
  ...
  public getErrorStatistics(): {
    ...
    const recentErrors = allErrors
      .filter(e => Date.now() - new Date(e.timestamp).getTime() < 24 * 60 * 60 * 1000) // Ultime 24 ore
