...
export default function FullSidebar({ activeStep, onStepChange }: { 
  activeStep: string;
    ...
    <div className="relative">
      ...
      >
        <SidebarContent className="overflow-hidden bg-transparent">
          {/* Header con Logo */}
          <div className={`flex items-center justify-center transition-all duration-300 ease-out ${isCollapsed ? 'p-3' : 'p-6'}`}>
            {!isCollapsed ? (
          ...
          {/* Menu di Navigazione */}
          <SidebarGroupContent className={`transition-all duration-300 ${isCollapsed ? 'px-1 py-2' : 'px-4 py-2'}`}>
            <SidebarMenu className="space-y-1">
              {SIDEBAR_STEPS.map((item, index) => {
                const isActive = activeStep === item.key;
                
                return (
                  <SidebarMenuItem key={item.key} className="transition-all duration-300" 
                    ...
                    >
                      <div className={`flex items-center w-full relative z-10 ${isCollapsed ? 'justify-center' : 'gap-4'}`}>
                        <div className={`
