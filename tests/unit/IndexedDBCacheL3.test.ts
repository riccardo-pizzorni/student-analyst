...
class MockIDBVersionChangeEvent
  extends MockEvent
...
class MockIDBRequest implements IDBRequest<Record<string, unknown>> {
  ...
  onsuccess:
    | ((this: IDBRequest<Record<string, unknown>>, ev: Event) => void)
    | null;
  onerror:
    | ((this: IDBRequest<Record<string, unknown>>, ev: Event) => void)
    | null;
...
class MockIndex implements IDBIndex {
  ...
  ): IDBRequest<Record<string, unknown> | undefined> {
...
class MockDatabase implements IDBDatabase {
  ...
  createObjectStore(
    ...
    const store = new MockObjectStore<Record<string, unknown>>(
      name,
      options || {},
      transaction
    );
    ...
    this.objectStoreNames = new MockDOMStringList(
      Array.from(this.objectStores.keys())
    );
    return store;
  }
...
class MockIndexedDB implements IDBFactory {
  ...
  databases(): Promise<IDBDatabaseInfo[]> {
    return Promise.resolve(
      Array.from(this._databases.values()).map(db => ({
        name: db.name,
        version: db.version,
      }))
    );
