...
describe('AutomaticCleanupService', () => {
  ...
  beforeEach(() => {
    ...
    mockIndexedDBCache.getStats.mockResolvedValue({
      hitCount: 0, missCount: 0, totalSize: 3000, entryCount: 0
    });
  ...
  describe('Initialization', () => {
    it('should initialize successfully with all components', async () => {
      ...
      expect(console.log).toHaveBeenCalledWith('🧹 Inizializzazione AutomaticCleanupService...');
      expect(console.log).toHaveBeenCalledWith('✅ AutomaticCleanupService inizializzato con successo');
    });
    ...
    it('should load existing LRU tracker data', async () => {
      const existingLRU = JSON.stringify([['L1:key1', 123456], ['L2:key2', 789012]]);
  ...
  describe('LRU Cleanup Operations', () => {
    it('should perform LRU cleanup on L1 cache', async () => {
      ...
      mockMemoryCache.getStats.mockReturnValue({
        hits: 100, misses: 20, currentEntries: 50,
        memoryUsage: 9500, size: 50
      });
    ...
    it('should perform LRU cleanup on L2 cache', async () => {
      ...
      mockLocalStorageCache.getStats.mockReturnValue({
        hits: 50, misses: 10, currentEntries: 30,
  ...
  describe('Service Shutdown', () => {
    it('should shutdown gracefully', () => {
      ...
      expect(mockClearTimeout).toHaveBeenCalled();
      expect(console.log).toHaveBeenCalledWith('🔥 AutomaticCleanupService shutdown completato');
    });
