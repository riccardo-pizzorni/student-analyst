  ...
  onRunComplete(_contexts, results) {
    try {
      ...
      if (results.coverageMap) {
        ...
        if (coverage) {
          console.log(`${symbols.info} Statements: ${this.getCoverageColor(coverage.statements?.pct || 0)}`);
          console.log(`${symbols.info} Branches: ${this.getCoverageColor(coverage.branches?.pct || 0)}`);
          console.log(`${symbols.info} Functions: ${this.getCoverageColor(coverage.functions?.pct || 0)}`);
          console.log(`${symbols.info} Lines: ${this.getCoverageColor(coverage.lines?.pct || 0)}`);
        }
      }
