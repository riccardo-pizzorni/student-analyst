...
test.describe('🎯 Realistic User Journeys', () => {
  ...
  test('✅ Application Loads Successfully', async ({ page }) => {
    ...
    expect(errors.filter(e => !e.includes('404') && !e.includes('network'))).toHaveLength(0);
  });
  ...
  test('📊 Financial Data Display', async ({ page }) => {
    ...
    const financialElements = [
      ...
      '[class*="chart"]', '[class*="portfolio"]', '[class*="data"]',
      // Common UI elements
      'button', 'input'
    ];
  ...
  test('🔄 Navigation & Interaction', async ({ page }) => {
    ...
    const visibleInteractiveElements = await page.locator('button:visible:enabled, a:visible, input:visible:enabled').all();
