...
test.describe('TIER 1 - Smoke Testing (100% Pass Required)', () => {
  ...
  test('Body element visibile (CSS rendering OK)', async ({ page }) => {
    ...
    console.log(
      `✅ TIER 1.2: Body CSS rendering PASSED (display: ${bodyStyles.display}, visibility: ${bodyStyles.visibility}, children: ${bodyStyles.children})`
    );
  });
  ...
  test('Main navigation presente e accessibile', async ({ page }) => {
    ...
    console.log(
      `✅ TIER 1.3: Main navigation PASSED (Content: ${hasMainContent}, Buttons: ${buttonCount})`
    );
  });

  test('No critical console errors (<5 errori non fatali max)', async ({
    page,
  }) => {
    const errors: string[] = [];
    ...
    expect(criticalErrors.length).toBeLessThan(5);
    console.log(
      `✅ TIER 1.4: Console errors check PASSED (${criticalErrors.length}/5 critical errors)`
    );
  });
  ...
  test('Basic performance threshold (<20s load iniziale)', async ({ page }) => {
    ...
    console.log(
      `✅ TIER 1.5: Performance threshold PASSED (Load: ${loadTime}ms, Memory: ${(memoryUsage / 1024 / 1024).toFixed(2)}MB)`
    );
  });
  ...
  test('Core UI elements render properly', async ({ page }) => {
    ...
    const domInfo = await page.evaluate(() => {
      return {
        ...
        hasText:
          document.body.innerText.includes('Student') ||
    ...
    console.log(
      '✅ TIER 1.6: Core UI elements rendering PASSED (Basic DOM structure verified)'
    );
  });
});
