 ...
 * 
 * [REFAC] Questo servizio ora supporta dependency injection per tutte le API browser e servizi cache.
 * Tutti gli accessi a window/localStorage/sessionStorage/indexedDB/cacheService sono ora iniettati.
 * Questo migliora la testabilità e l'isolamento dei test unitari.
 */

import { cacheService } from './CacheService';
import { ICacheService, IIndexedDB, IStorage, IWindow } from './interfaces/IStorage';
...
export class CacheQualityService {
  ...
  constructor(
    ...
    this.localStorage = deps?.localStorage || (window.localStorage as unknown as IStorage);
    this.sessionStorage = deps?.sessionStorage || (window.sessionStorage as unknown as IStorage);
    this.indexedDB = deps?.indexedDB || (window.indexedDB as unknown as IIndexedDB);
    this.cacheService = deps?.cacheService || (cacheService as ICacheService);
    this.win = deps?.win || (window as unknown as IWindow);
  ...
  private setupDefaultRules(): void {
    ...
    this.addRule({
      ...
      description: 'Validates relationships between different data fields',
      dataType: 'all',
      severity: 'medium',
      enabled: true,
      autoInvalidate: false,
  ...
  private async handleQualityIssues(
    ...
    switch (action) {
      ...
      case 'refresh':
        ...
        this.win.dispatchEvent(new CustomEvent('cache-refresh-requested', {
    ...
    this.win.dispatchEvent(new CustomEvent('cache-quality-issue', {
      detail: event
    }));
  }
    ...
    if (data.timestamp) {
      ...
      if (age > maxAge) {
        issues.push({
          type: 'stale_data',
          severity: 'high',
          message: `Data is ${Math.round(age / (60 * 60 * 1000))} hours old`,
          actualValue: new Date(data.timestamp).toISOString(),
  ...
  private startQualityMonitoring(): void {
    this.checkInterval = this.win.setInterval(async () => {
  ...
  dispose(): void {
    if (this.checkInterval) {
      this.win.clearInterval(this.checkInterval);
    }
