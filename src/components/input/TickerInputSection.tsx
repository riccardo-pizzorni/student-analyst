...
import { Plus, X, TrendingUp, AlertTriangle, CheckCircle, Search } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-400">Aggiungi i ticker dei titoli da analizzare</p>
      </div>
        ...
        <div className="bg-slate-800/30 rounded-xl border border-slate-700/50 overflow-hidden">
          <div className="p-6 border-b border-slate-700/50">
            <h4 className="font-semibold text-slate-300 flex items-center gap-2">
              ...
              Portafoglio ({tickers.filter(t => t.status === 'valid').length} titoli validi)
            </h4>
          </div>
          
          <div className="divide-y divide-slate-700/30">
            {tickers.map((ticker) => (
              <div key={ticker.symbol} className="p-4 hover:bg-slate-700/20 transition-colors">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold text-sm ${
                      ticker.status === 'valid' ? 'bg-green-500/20 text-green-300' :
                      ticker.status === 'invalid' ? 'bg-red-500/20 text-red-300' :
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        {ticker.status === 'valid' && <CheckCircle size={14} className="text-green-400" />}
                        {ticker.status === 'invalid' && <AlertTriangle size={14} className="text-red-400" />}
                      </div>
                  ...
                  <div className="flex items-center gap-4">
                    {ticker.status === 'valid' && ticker.price && (
                      <div className="text-right">
                        <div className="font-semibold text-slate-200">${ticker.price.toFixed(2)}</div>
                        <div className={`text-sm ${
                          ticker.change && ticker.change > 0 ? 'text-green-400' : 'text-red-400'
                        }`}>
          ...
          {tickers.filter(t => t.status === 'valid').length >= 3 && (
            <div className="p-4 bg-green-500/5 border-t border-green-500/20">
              <div className="flex items-center gap-2 text-green-300 text-sm">
                <CheckCircle size={16} />
                Portafoglio pronto per l'analisi ({tickers.filter(t => t.status === 'valid').length} titoli)
              </div>
            </div>
          )}
        </div>
