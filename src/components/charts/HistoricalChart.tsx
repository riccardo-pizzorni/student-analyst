      ...
      labels: {
        ...
        generateLabels: chart => {
          ...
          return datasets.map((dataset, index) => {
            ...
            const hasGaps =
              dataset.data &&
              dataset.data.filter(v => v === null).length >
                dataset.data.length * 0.1;
...
function rollingAverage(
  ...
  for (let i = 0; i < arr.length; i++) {
    if (
      i < window - 1 ||
      arr.slice(i - window + 1, i + 1).some(v => v == null || isNaN(Number(v)))
    ) {
      result.push(null);
    } else {
...
const HistoricalChart = () => {
  ...
  const handleInfoClick = () => {
    toast({
      ...
      description:
        "Questo grafico mostra l'andamento storico dei prezzi con indicatori tecnici. Usa gli switch per mostrare/nascondere indicatori. Puoi zoomare e fare pan sul grafico.",
    });
  };
  ...
  const missingTickers = useMemo(() => {
    if (
      !analysisResults?.metadata?.symbols ||
      !filteredDatasets ||
  ...
  const getChartStats = () => {
    if (
      !analysisResults?.historicalData?.datasets ||
      analysisResults.historicalData.datasets.length === 0
    ) {
      return null;
    }
    const priceDataset = analysisResults.historicalData.datasets.find(
      d => d.label?.includes('Prezzo') && !d.label?.includes('SMA')
    );
    if (!priceDataset || !priceDataset.data || priceDataset.data.length === 0) {
      return null;
    }
    // Fallback robusto: trova il primo e ultimo valore non null
    const prices = priceDataset.data.filter(
        ...
        <div className="space-y-3">
          ...
          {incompleteSeries.length > 0 && (
            <div className="p-3 bg-yellow-900/60 text-yellow-200 rounded-lg text-sm flex items-center gap-2">
              ...
              <span>
                Attenzione: dati disponibili solo fino a una certa data per{' '}
                <strong>{incompleteSeries.join(', ')}</strong>. Verifica la
                copertura storica del titolo.
              </span>
            </div>
          )}
          ...
          {missingTickers.length > 0 && (
            <div className="p-3 bg-orange-900/60 text-orange-200 rounded-lg text-sm flex items-center gap-2">
              ...
              <span>
                Ticker <strong>{missingTickers.join(', ')}</strong> non ha dati
                disponibili per il periodo selezionato. Verifica il simbolo o
                prova un periodo diverso.
              </span>
            </div>
          )}
          ...
          {hasTemporalGaps && (
            <div className="p-3 bg-blue-900/60 text-blue-200 rounded-lg text-sm flex items-center gap-2">
              ...
              <span>
                Rilevati buchi temporali significativi nei dati. Potrebbe essere
                dovuto a IPO recente, merge aziendale o cambio di simbolo.
              </span>
            </div>
          )}
        </div>
        ...
        <div className="p-4 bg-slate-800/30 rounded-lg border border-slate-700/50">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-sm text-slate-400">
            <div>
              ...
              <div className="mt-1">
                {analysisResults.metadata.symbols.join(', ')}
              </div>
            </div>
            <div>
              <span className="font-medium text-slate-300">Periodo:</span>
              <div className="mt-1">
                {analysisResults.metadata.period.start} -{' '}
                {analysisResults.metadata.period.end}
              </div>
            </div>
            <div>
        ...
        <div className="p-4 bg-slate-800/30 rounded-lg border border-slate-700/50">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="text-center p-3 bg-slate-700/30 rounded-lg">
              ...
              <div
                className={`text-lg font-bold ${stats.changePercent.startsWith('-') ? 'text-red-400' : 'text-green-400'}`}
              >
                {stats.changePercent}%
              </div>
            </div>
            ...
            >
              ...
              >
                ...
                <span className="absolute left-1/2 top-full z-10 hidden group-focus:block group-hover:block bg-slate-800 text-xs text-slate-200 rounded p-2 w-56 shadow-lg">
                  RSI: Indice di forza relativa, misura la velocità e il
                  cambiamento dei movimenti di prezzo, range 0-100. Sotto 30:
                  ipervenduto, sopra 70: ipercomprato.
                </span>
              </span>
            </Label>
            {showRSI && (
        ...
        {isLoading ? (
          <div className="w-full h-full flex items-center justify-center">
            <div className="text-center space-y-4">
              ...
              <p className="text-sm text-slate-400">
                Recupero dati storici da Yahoo Finance
              </p>
            </div>
          </div>
        ) : error ? (
          ...
          <div className="w-full h-full flex items-center justify-center">
            <div className="text-center space-y-4">
              ...
              <p className="text-slate-400">
                Avvia un'analisi per visualizzare l'andamento storico dei
                prezzi.
              </p>
            </div>
          </div>
